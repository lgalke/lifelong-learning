...
OUTFILE="results/revision-results/open_DOC_CW_fixed-threshold-with-SGC-base.csv"
...
for SEED in 10 11 12 13 14; do
	for DOC_THRESHOLD in "0.75"; do
		...
		for i in ${!HPARAMS[@]}; do
			echo "${HPARAMS[$i]}"
			echo "$OLG_ARGS"
			python3 run_experiment_new.py ${HPARAMS[$i]} --seed "$SEED" --model sgnet --backend geometric --n_hidden 32 $ARGS $PRETRAIN_ARGS --dataset "$DATA" $OLG_ARGS --save "$OUTFILE"
		done
	done
done
